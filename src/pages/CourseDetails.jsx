...
import { 
  ...
  orderBy, limit, onSnapshot, addDoc, deleteDoc 
} from "firebase/firestore";
import { 
  FiPlayCircle, FiClock, FiGlobe, FiAward, FiCheck, 
  FiUsers, FiStar, FiChevronDown, FiDownload,
  ...
  FiSmartphone, FiMonitor, FiChevronLeft, FiExternalLink
} from "react-icons/fi";
import { FaChalkboardTeacher, FaCertificate, FaRegPlayCircle, FaLaptop } from "react-icons/fa";
...
const CourseDetails = () => {
      ...
      try {
        ...
        if (user) {
          try {
            ...
            setWishlisted(!wishlistSnap.empty);
          } catch (error) {
            console.log("Enrollment/wishlist check skipped:", error.message);
          }
        }

        // Fetch lectures with real-time listener
  ...
  const handleEnroll = async () => {
    ...
    if (enrolled) {
      ...
      const isFree = course?.price === 0 || course?.discountPrice === 0;
  ...
  const handleAddToWishlist = async () => {
    ...
    try {
      if (wishlisted) {
        ...
        const wishlistSnap = await getDocs(wishlistQuery);
        wishlistSnap.docs.forEach(async (doc) => {
  ...
  const handleShareCourse = () => {
    ...
    const shareText = `Check out this amazing course: ${course?.title}`;
  ...
  const calculateTotalDuration = () => {
    ...
    return Math.round(totalLectures * 0.5); // Assuming 30 minutes per lecture
  };

  const calculateTotalLectures = () => {
    return Object.values(lectures).flat().length;
  };
  ...
  return (
    <>
      <Helmet>
        <title>{course.title} - Learnify</title>
        <meta name="description" content={course.description || "Learn new skills with expert-led courses"} />
          ...
          <div className="max-w-7xl mx-auto px-4 lg:px-8 py-12">
            ...
            <nav className="mb-6">
              <ol className="flex items-center space-x-2 text-sm">
                ...
                <li>
                  <button 
                    onClick={() => navigate(`/category/${course.category?.toLowerCase().replace(/\s+/g, '-')}`)}
            ...
            <div className="grid lg:grid-cols-3 gap-8">
              ...
              <div className="lg:col-span-2 space-y-6">
                ...
                <div className="flex flex-wrap items-center gap-6">
                  ...
                  <div className="flex items-center gap-2">
                    <FiUsers />
                    <span>{course.enrolledStudents?.toLocaleString() || course.studentsEnrolled?.toLocaleString() || "5,432"} students</span>
                  </div>
              ...
              <div className="sticky top-24 self-start">
                <div className="bg-white rounded-2xl shadow-2xl overflow-hidden border border-slate-200">
                    ...
                    <div 
                      ...
                      onClick={() => {
                        if (course.previewVideo) {
                          window.open(course.previewVideo, '_blank');
                        } else {
                          toast.info("Preview video not available");
                        }
                      }}
                    >
                  ...
                  <div className="p-6">
                    ...
                    <div className="mb-6">
                      {isFree ? (
                        <div>
                          ...
                          <p className="text-slate-500 text-sm mt-1">Enroll for free</p>
                        </div>
                    ...
                    {/* Enrollment Progress */}
                    {enrolled && enrollmentProgress > 0 && (
                      <div className="mb-6">
                        <div className="flex justify-between text-sm mb-1">
                          <span className="text-slate-600">Your Progress</span>
                          <span className="font-bold">{enrollmentProgress}%</span>
                        </div>
                    ...
                    <div className="space-y-3">
                      ...
                      {!isFree && !enrolled && (
                        <button 
                          ...
                          className="w-full py-3 px-6 border-2 border-red-600 text-red-600 rounded-xl font-bold hover:bg-red-50 transition-colors"
                        >
                          Buy Now
                        </button>
                      )}
                      ...
                      <button 
                        ...
                        className={`w-full py-3 px-6 border border-slate-200 rounded-xl font-bold transition-colors flex items-center justify-center gap-2 ${
                          wishlisted ? 'bg-red-50 text-red-600 border-red-200' : 'hover:bg-slate-50 text-slate-600'
                        }`}
                      >
                        <FiBookmark className={wishlisted ? "fill-current" : ""} /> 
                    ...
                    <div className="mt-6 pt-6 border-t border-slate-100">
                      ...
                      <ul className="space-y-2">
                        ...
                        <li className="flex items-center gap-2 text-slate-600">
                          <FaLaptop className="text-red-600" /> Access on desktop
                        </li>
                        <li className="flex items-center gap-2 text-slate-600">
                          <FiGlobe className="text-red-600" /> Access from anywhere
                        </li>
                      </ul>
                    </div>
                    ...
                    <div className="mt-6 flex gap-3 relative">
                      <button 
                        ...
                        className="flex-1 py-2 border border-slate-200 text-slate-600 rounded-lg font-bold hover:bg-slate-50 flex items-center justify-center gap-2"
                      >
                        <FiShare2 /> Share
                      </button>
                      ...
                      {/* Share Menu Dropdown */}
                      {showShareMenu && (
                        <div className="absolute bottom-full left-0 mb-2 bg-white border border-slate-200 rounded-lg shadow-xl p-3 w-64">
                          <div className="flex justify-between mb-2">
                            <span className="font-bold text-slate-800">Share via</span>
                            <button 
                          ...
                          <div className="mt-3 pt-3 border-t border-slate-100">
                            <button 
                              ...
                              className="w-full py-2 border border-slate-200 rounded-lg text-slate-600 hover:bg-slate-50"
                            >
                              Copy Link
                            </button>
                          </div>
                ...
                <div className="border border-slate-200 rounded-xl overflow-hidden">
                  {Object.keys(lectures).length > 0 ? (
                    ...
                    <div className="p-8 text-center">
                      <FiBook className="text-slate-300 text-4xl mx-auto mb-3" />
                      <p className="text-slate-500">Course content will be added soon</p>
                    </div>
                  )}
                </div>
              </section>
                  ...
                  <div className="flex flex-col md:flex-row gap-6 items-start">
                    <div className="flex-shrink-0">
                      <div className="w-24 h-24 rounded-full overflow-hidden border-4 border-white shadow-lg">
                        <img 
                          src={instructor?.photoURL || `https://api.dicebear.com/7.x/avataaars/svg?seed=${instructor?.name || 'instructor'}`}
                          alt={instructor?.name}
                          className="w-full h-full object-cover"
                        />
                      </div>
                ...
                <div className="grid md:grid-cols-2 gap-6">
                  {reviews.length > 0 ? (
                          ...
                          <div>
                            ...
                            <div className="flex items-center gap-1">
                              {[1, 2, 3, 4, 5].map((star) => (
                                <FiStar 
                                  key={star} 
                                  className={`${star <= (review.rating || 5) ? "text-amber-400 fill-current" : "text-slate-300"}`} 
                                />
                              ))}
                            </div>
                          </div>
                        </div>
                        <p className="text-slate-700 line-clamp-3">{review.comment || "Great course!"}</p>
                    ...
                    <div className="col-span-2 text-center py-8">
                      <FiMessageCircle className="text-slate-300 text-4xl mx-auto mb-3" />
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <p className="text-sm text-slate-600">
                      {isFree ? "Yes, this course is completely free!" : "This course offers free preview lessons. Full access requires enrollment."}
                    </p>
                  </div>
                  <div>
                  ...
                  <button 
                    onClick={() => navigate(`/course/${courseId}/faq`)}
                    className="w-full text-center text-red-600 font-bold hover:text-red-700"
                  >
                    See all FAQs
                  </button>
                </div>
                    ...
                    {relatedCourses.map((relatedCourse) => (
                      ...
                      >
                        <div className="w-16 h-16 rounded-lg overflow-hidden flex-shrink-0">
                          <img 
                            src={relatedCourse.thumbnail || "https://images.unsplash.com/photo-1517077304055-6e89abbf09b0?ixlib=rb-1.2.1&auto=format&fit=crop&w=200&q=80"}
                        ...
                        <div>
                          ...
                          <p className="text-sm text-slate-500">By {relatedCourse.instructorName || "Expert"}</p>
                          <div className="flex items-center gap-2 mt-1">
                            ...
                            {relatedCourse.discountPrice && relatedCourse.price > relatedCourse.discountPrice && (
                              <span className="text-sm text-slate-400 line-through">₹{relatedCourse.price}</span>
                            )}
                          </div>
                        </div>
                      </div>
        ...
        <div className="bg-gradient-to-r from-red-600 to-rose-600 text-white py-12">
          <div className="max-w-7xl mx-auto px-4 lg:px-8 text-center">
            ...
            <p className="text-xl mb-8 max-w-2xl mx-auto">
              Join {course.enrolledStudents?.toLocaleString() || course.studentsEnrolled?.toLocaleString() || "5,432"} students who have already enrolled
            </p>
            <button
              onClick={handleEnroll}
              className="px-8 py-4 bg-white text-red-600 rounded-xl font-bold text-lg hover:bg-slate-100 transition-colors shadow-2xl"
            >
              {enrolled ? 'Continue Learning' : isFree ? 'Enroll for Free' : 'Enroll Now'}
            </button>
          </div>
        </div>
      </div>
